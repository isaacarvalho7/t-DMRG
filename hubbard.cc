...
/*
    ...
    for(int j = 1; j <= L; ++j)
        {
        psiGS.position(j);
        upd(j-1) = eltC(dag(prime(psiGS(j),"Site"))*op(sites,"Nup",j)*psiGS(j)).real();
        dnd(j-1) = eltC(dag(prime(psiGS(j),"Site"))*op(sites,"Ndn",j)*psiGS(j)).real();
        }
...
//Está parte abaixo calculava a mcdw do GS, mas já redefine com novas variáveis acima
/*
auto mcdw = 0.;

for(int j = 0; j < L; ++j)
{
    mcdw += pow(-1,j+1)*((upd(j)+dnd(j))-1);
}


printfln("EnergyGS = %.20f",energy); //Prints energy
printfln("L = ", L);
printfln("Npart = ", Npart);

auto moc = sqrt(pow((mcdw/L),2));

printfln("|mcdw_GS| = ", moc );
*/



//The function below evaluates the physical time of the routine from this point
/*
char num_char[MAX_DIGITS + sizeof(char)];
    ...
    std::cout << "\nThe temporal evolution took  " << time_span.count() << " seconds.";
    std::cout << std::endl;
*/
